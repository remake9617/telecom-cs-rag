...
## D34. ReAct Agent + MCP 定位 = 阶段二加分项，可砍
- **决策**：阶段二的**加分项，可砍**，不进论文核心章节。
...
## D37. 评估体系归属 = cs-knowledge 承载评测执行，cs-stats 只读展示
- **决策**：RAG 评估体系（用例管理、评测执行、指标计算、结果落库）放 `cs-knowledge` 新增的 `eval` 包；`cs-stats` 只新增读 `eval_result` 表的 `@Select` 供看板展示（`GET /api/stats/eval-runs`）。
- **理由**：评估必须**驱动检索**（用不同 mode/kbId/minScore 跑同一批评测集才能产出对比数据），必然要调 `RetrievalService`；而 D24 明确 cs-stats 只读直查表、不依赖其它模块 Service，放进 cs-stats 就必须破 D24 的依赖方向。评估检索质量本属检索模块的职责范围（如同单元测试属于被测模块）；cs-stats 只读 `eval_result` 完全符合其既有定位，D24 无损。
- **被放弃的方案**：① 放 cs-stats（破 D24 依赖方向）；② 新建 `cs-eval` 模块（要改根 pom 与 CONVENTIONS §2、增加 reactor 项目，成本高于收益）。**本条推翻了统筹早期「A1 主要在 cs-stats 内新增，耦合最小」的判断**——当时未细想评估需要驱动检索。
- **日期**：2026-09-09

## D38. 评估指标范围 = 检索侧确定性指标优先，生成侧留批次 1
- **决策**：本轮只实现可确定性计算的检索侧六项指标（`recall_at_k`/`precision_at_k`/`mrr`/`ndcg`/`hit_rate`/`ref_precision`）；答案忠实度（faithfulness）与幻觉率**不做**，`eval_result` 表**不建幻觉率列**。DISLIKE 回流的用例**默认不参与指标计算**，仅作 bad case 池。
- **理由**：生成侧指标需额外调 LLM 做判定，判定本身不稳定且要设计评判 prompt，属批次 1 深化内容。**不建幻觉率列**是为了避免留下一个永远为 null 的空列误导后来者（`schema.sql` 末尾的预留注释提到过该字段，本轮明确不建）。DISLIKE 样本是反例、没有正向标注，若参与 recall 计算会因期望集为空而虚高。
- **被放弃的方案**：① 本轮一并做忠实度/幻觉率（判定不稳定，会把不可靠数字写进论文）；② 建列留空（空列误导）；③ DISLIKE 参与指标（recall 虚高）。
- **日期**：2026-09-09

## D39. ref_precision 口径 = 文档粒度，且不可与 chunk 级 precision@k 直接比较
- **决策**：`ref_precision`（引用准确率）按**文档粒度**计算——返回给用户的 `references` 里 `docId` 落在期望文档集内的比例，期望文档缺失时由 chunk id 反查 `kb_chunk_meta` 补齐；与 chunk 级的 `precision_at_k` 是两个独立指标。
- **理由**：引用溯源在前端是按「文档标题 + 原文片段」展示的，用户关心的是「引用的文档对不对」，文档粒度更贴近用户感知，直接对应答辩要讲的「引用溯源可信度」；chunk 级 precision 更适合评估检索精度本身。
- **必须记录的口径警示**：文档粒度**系统性高于** chunk 级（一个文档含多个 chunk，命中任一即算对），**两个数字不可直接比较、不可在论文里并列陈述而不加说明**，否则会被质疑。
- **日期**：2026-09-09

## D40. 评测集标注必须独立于检索结果（当前种子数据的已知局限）
- **决策**：批次 1 扩充评测集时，期望集（`expected_chunk_ids`/`expected_doc_ids`）必须由**人工读文档后独立标注**，标注时**不得查看任何检索通道的输出**。
- **理由**：本轮 3 条种子用例的期望集取自默认通道（`HYBRID_RERANK`）的 top-2，构成**循环论证**——默认通道必然 recall=1.0，其它通道也系统性偏高；实测四个 mode 的 recall 全为 1.0、precision 全为 0.400 正是这个原因。**这样的数据不能用于证明「混合检索优于单一通道」**，答辩时被问一句「期望集怎么标注的」即崩。当前唯一有效的信号是 `ndcg`（BM25 为 0.946，把一条期望 chunk 排后）与 `ref_precision` 的通道间差异，且样本仅 3 条不足以下结论。
- **附带观察（批次 1 需验证）**：`HYBRID` 的 `ref_precision`(0.733) **高于** `HYBRID_RERANK`(0.667)，即 rerank 在种子上反而降低了引用文档覆盖率。样本太小不能定论，但若规模化后仍成立，说明 rerank 优化的是「答案相关性」而非「引用文档覆盖率」，两者不是一回事——这会是有分量的发现。
- **被放弃的方案**：用默认通道输出自动标注（就是本轮的做法，已证明不可用于结论）。
- **日期**：2026-09-09

## D41. SSE 端点的错误出口 = 一律走 SSE error 事件，不用 JSON 响应体
- **决策**：SSE 端点的一切错误（参数校验失败、限流拒绝、业务错误）都以 SSE `event: error` + `{code, message}` 下发；**SSE 端点不用 DTO 层 `@Valid`**，参数校验放服务层。非 SSE 端点仍走 JSON + `GlobalExceptionHandler`。已写入 `CONVENTIONS.md` §4。
- **理由**：客户端以 `Accept: text/event-stream` 请求，JSON 错误体会被 406 拒绝或被 SSE 解析器当正文渲染成乱码。本条由两路独立踩坑后收敛：DEF-088（`@Valid` 失败返回 JSON 被前端静默吞掉）与路 10 的限流实现（原设计「HTTP 200 + code=1005」在 SSE 端点不可用，改为 error 事件）。
- **被放弃的方案**：① 定制 SSE 感知的校验失败处理器（复杂度高收益低）；② 改前端让它能解析 JSON 错误体（为后端的错误设计买单）。
- **日期**：2026-09-09

## D42. 限流 = Redis+Lua 滑动窗口拒绝式；幂等仅覆盖建单；热点缓存不实现
- **决策**：限流用 Redis + Lua 的**滑动窗口**（ZSET 存时间戳，一个脚本内完成 `ZREMRANGEBYSCORE`+`ZCARD`+`ZADD`+`PEXPIRE`），超阈抛已有的 `1005 RATE_LIMITED`；维度为 `userId` + 端点，默认窗口 60 秒 / 10 次。幂等**只用于 `POST /api/ticket`**，键 `idem:ticket:{userId}:{sha256(conversationId|question)}`、`SET NX EX` 原子占位、value 存 ticketId，重复提交**返回同一张工单而非报错**，窗口 60 秒。
- **理由**：滑动窗口无固定窗口的边界突发问题（切换瞬间可放行 2 倍流量）；Lua 保证原子性，`GET`+`INCR` 两步在并发下会超发使限流形同虚设。幂等只覆盖建单是因为问答重复提问是合法行为不该拦、`POST /api/feedback` 已是 upsert 改票语义天然幂等；用「userId+内容哈希」做键而非客户端传 requestId，是为了**不牵动前端**。
- **被放弃的方案**：① Redis 公平排队（需异步等待与位置回报，SSE 无法排队，前端要改，成本高一个量级——列为可选增强）；② 固定窗口计数（边界突发）；③ 三个写接口全加幂等并要求前端传 requestId（越出「不碰 frontend」边界）。
- **附带记录**：`hot:qa:{hash}` 热点问答缓存**评估后不实现**——精确 hash 命中率极低，语义相似缓存需再上一套向量检索成本远超收益，缓存答案无 `messageId` 会让前端点赞失效，带 `ticket_hint` 的响应还不能缓存（破坏 D9 方案①的自动建单语义）。
- **日期**：2026-09-09

## D43. SSE 取消联动 = takeUntilOther 而非直接 cancel 上游订阅
- **决策**：客户端断开后，通过 `SseEmitter` 的 `onCompletion`/`onTimeout`/`onError` 回调触发一个 kill 信号，用 `takeUntilOther(killSignal)` 终止 Flux；**不直接 `Disposable.dispose()` 取消上游订阅**。断开路径的行为是：终止后续生成 → 已累积的部分内容落库（走 `trySavePartial`，content 为空时入口守卫跳过 insert）→ **不推任何事件**（连接已断）→ 日志记 info 而非 error。
- **理由**：**`Flux.toStream()` 的阻塞迭代器在外部直接 cancel 上游订阅时不会收到终止哨兵，会永久挂起**（首版实现即踩此坑，表现为 kill 客户端后线程卡死）；改用 `takeUntilOther` 后迭代同毫秒退出。「断开」与「模型失败」是两种不同情形：模型失败要推 error 事件告知用户（D31），断开时客户端已不存在、推事件只会触发 `ResponseBodyEmitter has already completed` 警告（DEF-078/081 的现象），但已生成部分仍应落库，用户刷新后能看到那半截回答。
- **被放弃的方案**：① 直接 `dispose()`（迭代器永久挂起）；② 断开时也推 error 事件（连接已断，只会刷警告）；③ 断开时丢弃已生成内容（用户刷新后什么都看不到，体验更差）。
- **日期**：2026-09-09
