 ...
 * 做只读聚合 SQL，而不依赖 cs-qa / cs-ticket 的 Service——cs-stats 按依赖契约
 * 只依赖 cs-framework（CONVENTIONS 第 2 节），表级只读是数仓/统计模块的常规边界
 * （M3-report 说明）。写入仍由各归属模块负责，本 Mapper 只 SELECT。</p>
 */
@Mapper
