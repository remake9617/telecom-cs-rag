# =====================================================================
# 集成收口 v3：A 轮 27 端点全回归 + DEF-085/088 验证 + p9 演示数据重建
...
[IO.File]::WriteAllText("$PWD\test-data\p9-noq.json", '{"conversationId":null}', (New-Object Text.UTF8Encoding $false))
& curl.exe -s -N -X POST "$B/api/qa/chat/stream" -H 'Content-Type: application/json' -H "Authorization: Bearer $T" --data-binary '@test-data/p9-noq.json'
""
...
$exist = $kbList.data | Where-Object { $_.name -eq 'p9-演示知识库' } | Select-Object -First 1
if ($exist) { $kbId = $exist.id; "reuse kbId=$kbId" }
else {
  $kb = Invoke-RestMethod "$B/api/kb/bases" -Method Post -ContentType 'application/json;charset=utf-8' -Headers (Hdr $T) -Body ([Text.Encoding]::UTF8.GetBytes('{"name":"p9-演示知识库","description":"批次0 收口重建的演示库"}'))
  $kbId = $kb.data.id
...
"===== ⑥ qa SSE（q1，逐包时间戳）====="
[IO.File]::WriteAllText("$PWD\test-data\p9-q1.json", '{"question":"p9-129套餐包含多少流量","conversationId":null}', (New-Object Text.UTF8Encoding $false))
$sse1 = & curl.exe -s -N -X POST "$B/api/qa/chat/stream" -H 'Content-Type: application/json' -H "Authorization: Bearer $T" --data-binary '@test-data/p9-q1.json' | ForEach-Object { "$(Get-Date -Format 'HH:mm:ss.fff')|$_" }
...
"===== ⑦ qa SSE（q2 追问，复用会话）====="
[IO.File]::WriteAllText("$PWD\test-data\p9-q2.json", ('{"conversationId":' + $convId + ',"question":"p9-那它的通话分钟是多少"}'), (New-Object Text.UTF8Encoding $false))
$sse2 = & curl.exe -s -N -X POST "$B/api/qa/chat/stream" -H 'Content-Type: application/json' -H "Authorization: Bearer $T" --data-binary '@test-data/p9-q2.json' | ForEach-Object { "$(Get-Date -Format 'HH:mm:ss.fff')|$_" }
...
$tk = Invoke-RestMethod "$B/api/ticket" -Method Post -ContentType 'application/json;charset=utf-8' -Headers (Hdr $T) -Body ([Text.Encoding]::UTF8.GetBytes('{"question":"p9-手动创建的演示工单"}'))
"create code=$($tk.code) ticketId=$($tk.data.id) status=$($tk.data.status)"
$rp = Invoke-RestMethod "$B/api/ticket/$($tk.data.id)/reply" -Method Put -ContentType 'application/json;charset=utf-8' -Headers (Hdr $T) -Body ([Text.Encoding]::UTF8.GetBytes('{"reply":"p9-演示回复：您好，问题已处理。"}'))
...
$reg = Invoke-RestMethod "$B/api/auth/register" -Method Post -ContentType 'application/json;charset=utf-8' -Body ([Text.Encoding]::UTF8.GetBytes('{"username":"p9visitor","password":"p9Pass123456"}'))
...
$reg2 = Invoke-RestMethod "$B/api/auth/register" -Method Post -ContentType 'application/json;charset=utf-8' -Body ([Text.Encoding]::UTF8.GetBytes('{"username":"p9v2","password":"p9Pass123456"}'))
...
"===== ⑮ DELETE 会话（用即弃会话）====="
[IO.File]::WriteAllText("$PWD\test-data\p9-q3.json", '{"question":"p9-delete-test 一次性会话","conversationId":null}', (New-Object Text.UTF8Encoding $false))
$d1 = & curl.exe -s -N -X POST "$B/api/qa/chat/stream" -H 'Content-Type: application/json' -H "Authorization: Bearer $T" --data-binary '@test-data/p9-q3.json'
